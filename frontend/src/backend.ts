...
import type { NewsItem as _NewsItem, UserProfile as _UserProfile, UserRole as _UserRole } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async getAdminMessages(): Promise<Array<ContactMessage>> {
        if (this.processError) {
            try {
    ...
    async getContactInfo(): Promise<SchoolContactInfo> {
        if (this.processError) {
            try {
    ...
    async getNewsById(arg0: bigint): Promise<NewsItem | null> {
        if (this.processError) {
            try {
...
function from_candid_opt_n6(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_NewsItem]): NewsItem | null {
    return value.length === 0 ? null : value[0];
}
